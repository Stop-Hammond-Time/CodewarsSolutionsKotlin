package gap
//checks if numer is prime
fun isPrime(number:Long):Boolean {
    if (number % 2 == 0L) return false
    for (it in 2..number/4){
        if (number % (it*2-1) == 0.toLong()) return false
    }
    return true
}

// checks for prime in range
fun isPrimeInRange(start:Long,end:Long):Boolean {
...
fun gap(g:Int, m:Long, n:Long):LongArray {
    for (i in m..n){
        if (isPrime(i) && isPrime(i+g.toLong()) && isPrimeInRange(i+1,i+g-1)==false)
            return longArrayOf(i,i + g.toLong())
    }
    return longArrayOf()
}
